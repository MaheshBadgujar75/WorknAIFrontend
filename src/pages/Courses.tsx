...
const Courses: React.FC = () => {
  ...
  useEffect(() => {
    const loadCourses = async () => {
      try {
        ...
        const activeCourses = data.filter(course => course.isActive !== false);
  ...
  return (
    <div ref={pageRef} className="min-h-screen pt-32 pb-24 px-6 md:px-12">
      <section className="max-w-[1600px] mx-auto mb-16 relative">
        <div
          className={`absolute -top-20 -left-20 w-[600px] h-[600px] blur-[180px] rounded-full opacity-10 pointer-events-none ${isDarkMode ? "bg-cyan-500" : "bg-blue-200"
            }`}
        />
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <motion.p
            ...
            className={`text-lg md:text-xl font-medium italic transition-colors max-w-2xl ${isDarkMode ? "text-zinc-400" : "text-zinc-600"
              }`}
          >
        ...
        >
          <div className="flex flex-col md:flex-row items-center gap-6 w-full lg:w-auto">
            ...
            <div className="relative w-full md:w-80 group">
              ...
              {searchQuery && (
                <button
                  ...
                  className={`absolute right-5 top-1/2 -translate-y-1/2 ${isDarkMode ? "text-zinc-500 hover:text-white" : "text-zinc-400 hover:text-black"
                    }`}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              <span
                className={`text-[10px] font-black uppercase tracking-widest hidden md:block ${isDarkMode ? "text-zinc-400" : "text-zinc-600"
                  }`}
              >
                Mode:
              </span>
          ...
          {/* Results Count */}
          <div
            className={`flex items-center gap-4 px-6 border-l ${isDarkMode ? "border-zinc-700" : "border-zinc-200"
              }`}
          >
            <span
              className={`text-sm font-bold italic ${isDarkMode ? "text-zinc-400" : "text-zinc-600"
                }`}
            >
      ...
      <section className="max-w-[1600px] mx-auto">
        ...
        {error && !isLoading && (
          ...
          >
            ...
            <h3
              className={`text-3xl font-black font-syne uppercase italic mb-2 ${isDarkMode ? "text-white" : "text-black"
                }`}
            >
              Error Loading Courses
            </h3>
            <p
              className={`font-medium mb-8 ${isDarkMode ? "text-zinc-400" : "text-zinc-600"
                }`}
            >
              {error}
            </p>
        ...
        {isLoading && !error && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className={`h-[500px] rounded-[2.5rem] animate-pulse ${isDarkMode ? "bg-zinc-800" : "bg-zinc-100"
                  }`}
              >
                <div className="p-8 space-y-4">
                  <div
                    className={`h-6 rounded ${isDarkMode ? "bg-zinc-700" : "bg-zinc-200"
                      }`}
                  />
                  <div
                    className={`h-4 w-3/4 rounded ${isDarkMode ? "bg-zinc-700" : "bg-zinc-200"
                      }`}
                  />
                  <div
                    className={`h-4 w-1/2 rounded ${isDarkMode ? "bg-zinc-700" : "bg-zinc-200"
                      }`}
                  />
                </div>
              </div>
        ...
        {!isLoading && !error && (
          <AnimatePresence mode="popLayout">
            {filteredCourses.length > 0 ? (
              ...
              >
                <div
                  className={`w-24 h-24 mb-8 ${isDarkMode ? "text-zinc-600" : "text-zinc-300"
                    }`}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
