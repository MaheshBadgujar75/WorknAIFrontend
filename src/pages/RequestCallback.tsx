...
const RequestCallback: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className={`callback-glow absolute top-0 right-0 w-[300px] md:w-[500px] h-[300px] md:h-[500px] blur-[100px] md:blur-[150px] rounded-full opacity-10 pointer-events-none transition-colors duration-1000 ${isDarkMode ? 'bg-cyan-500' : 'bg-emerald-200'}`}></div>
      <div className={`callback-glow absolute bottom-0 left-0 w-[300px] md:w-[500px] h-[300px] md:h-[500px] blur-[100px] md:blur-[150px] rounded-full opacity-10 pointer-events-none transition-colors duration-1000 ${isDarkMode ? 'bg-fuchsia-500' : 'bg-rose-200'}`}></div>
      ...
      <div className="max-w-[1400px] w-full grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 relative z-10 items-center pt-24 pb-12 lg:pt-0 lg:pb-0">
        ...
        <div className="lg:col-span-7 space-y-6 lg:space-y-8">
          ...
          >
            <span className={`inline-block py-1.5 px-4 rounded-full text-[9px] font-black uppercase tracking-[0.3em] mb-4 border ${isDarkMode ? 'bg-zinc-900/50 border-cyan-500/30 text-cyan-400' : 'bg-white border-zinc-200 text-zinc-500'}`}>
            ...
            <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black font-syne leading-[0.85] lg:leading-[0.8] tracking-tighter uppercase italic mb-6">
              Expert <br />
              <span className={`text-transparent bg-clip-text ${isDarkMode ? 'bg-gradient-to-r from-cyan-400 via-blue-500 to-fuchsia-500' : 'bg-gradient-to-r from-rose-500 via-emerald-500 to-blue-500'}`}>Guidance</span>
              <br /> Awaits.
            </h1>
            <p className={`text-base md:text-xl font-medium leading-relaxed italic border-l-4 border-cyan-500 pl-6 max-w-lg transition-colors ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
              "Logic will get you from A to B. Imagination and skilled mentorship will take you everywhere."
            </p>
          </motion.div>

          <div className="grid grid-cols-2 gap-4 md:gap-8 pt-4 max-w-sm border-t border-zinc-500/10">
            <div className="space-y-1">
              <span className={`text-2xl md:text-3xl font-black font-syne ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>15m</span>
              <p className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-zinc-500">Wait Time</p>
            </div>
            <div className="space-y-1">
              <span className={`text-2xl md:text-3xl font-black font-syne ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>100%</span>
              <p className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-zinc-500">Direct Connect</p>
            </div>
          </div>
        </div>
        ...
        <motion.div
          ...
          className={`lg:col-span-5 justify-self-end w-full max-w-lg relative p-6 md:p-8 lg:p-10 rounded-[2.5rem] md:rounded-[3.5rem] refractive-border border liquid-glass shadow-2xl ${isDarkMode ? 'bg-zinc-950/40 border-white/5' : 'bg-white/40 border-white/20'
            }`}
        >
            ...
            {!isSuccess ? (
              ...
              >
                <div className="form-item">
                  <label className={`block text-[8px] font-black uppercase tracking-[0.3em] mb-1.5 ml-1 ${isDarkMode ? 'text-white' : 'text-zinc-500'}`}>Full Name</label>
                  <input
                    required
                    name="name"
                    type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="form-item">
                    <label className={`block text-[8px] font-black uppercase tracking-[0.3em] mb-1.5 ml-1 ${isDarkMode ? 'text-white' : 'text-zinc-500'}`}>Phone</label>
                    <input
                      required
                      name="phone"
                      type="tel"
                  ...
                  <div className={`form-item relative ${isDropdownOpen ? 'z-[100]' : 'z-[1]'}`} ref={dropdownRef}>
                    <label className={`block text-[8px] font-black uppercase tracking-[0.3em] mb-1.5 ml-1 ${isDarkMode ? 'text-white' : 'text-zinc-500'}`}>Choose Track</label>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <motion.svg
                        ...
                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className={`w-3 h-3 ${isDarkMode ? 'text-cyan-400' : 'text-zinc-400'}`}
                      >
                ...
                <div className="form-item z-[0]">
                  <label className={`block text-[8px] font-black uppercase tracking-[0.3em] mb-1.5 ml-1 ${isDarkMode ? 'text-white' : 'text-zinc-500'}`}>Preferred Slot</label>
              ...
              >
                <div className={`w-16 h-16 md:w-20 md:h-20 mx-auto rounded-full flex items-center justify-center ${isDarkMode ? 'bg-cyan-500/20' : 'bg-green-100'}`}>
                  <motion.svg
                    initial={{ pathLength: 0 }}
                    animate={{ pathLength: 1 }}
                    transition={{ duration: 1, ease: "easeInOut" }}
                    className={`w-8 h-8 md:w-10 md:h-10 ${isDarkMode ? 'text-cyan-400' : 'text-green-600'}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={3}
                ...
                <div>
                  <h3 className="text-2xl md:text-3xl font-black font-syne uppercase tracking-tight italic mb-2">Priority Logged</h3>
