...
const CourseCard: React.FC<Props> = ({ course }) => {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`group flex flex-col h-full rounded-[2.5rem] overflow-hidden border-2 transition-all duration-500 hover:shadow-2xl ${isDarkMode
    ...
    >
      ...
      <div className="relative h-72 overflow-hidden m-3 rounded-[1.8rem]">
        ...
        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
          ...
          {isHighDiscount && (
            <span
              className={`text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg ${isDarkMode ? "bg-cyan-600" : "bg-emerald-500"
                }`}
            >
              Best Value
            </span>
          ...
          {discountPercent > 0 && (
            <span
              className={`text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg ${isDarkMode ? "bg-fuchsia-600" : "bg-rose-500"
                }`}
            >
              {discountPercent}% OFF
            </span>
        ...
        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500">
          <span
            className={`px-3 py-1.5 text-white text-[10px] font-black rounded-full uppercase tracking-widest ${course.status === "Online"
                ? isDarkMode
                  ? "bg-cyan-700"
                  : "bg-blue-600"
      ...
      <div className="px-8 pb-8 pt-4 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-4">
          <h3
            className={`text-2xl font-bold font-syne leading-tight tracking-tight group-hover:text-cyan-400 transition-colors ${isDarkMode ? "text-zinc-100" : "text-zinc-900"
              }`}
          >
            {course.name}
          </h3>
        ...
        <p
          className={`text-sm mb-6 line-clamp-2 ${isDarkMode ? "text-zinc-400" : "text-zinc-600"
            }`}
        >
          {course.description}
        </p>
        ...
        <div className="space-y-6 mt-auto">
          {/* Pricing Card */}
          <div
            className={`p-5 rounded-2xl border transition-colors ${isDarkMode
          ...
          >
            <div className="flex items-end justify-between mb-3">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-3xl font-black ${isDarkMode ? "text-white" : "text-zinc-900"
                      }`}
                  >
                  ...
                  {course.originalPrice !== course.discountedPrice && (
                    <span
                      className={`text-sm line-through decoration-2 ${isDarkMode
              ...
              {discountPercent > 0 && (
                <div
                  className={`text-white text-[10px] font-black px-2 py-1 rounded-md mb-1 ${isDarkMode ? "bg-cyan-600" : "bg-emerald-500"
                    }`}
                >
            ...
            {/* GST and Total */}
            <div
              className={`pt-3 border-t border-dashed flex justify-between items-center ${isDarkMode ? "border-zinc-800" : "border-zinc-200"
                }`}
            >
              ...
              <span
                className={`text-xs font-black ${isDarkMode ? "text-cyan-400" : "text-emerald-500"
                  }`}
              >
          ...
          <Link
            to={`/course/${course.id}`}
            className={`group/btn relative overflow-hidden flex items-center justify-center gap-2 w-full py-5 rounded-2xl font-black text-sm uppercase tracking-widest transition-all ${isDarkMode
